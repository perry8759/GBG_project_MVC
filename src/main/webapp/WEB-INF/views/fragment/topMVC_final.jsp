	...
	<nav
		...
		<div class="container-fluid px-md-5">
			<a class="navbar-brand" href="<c:url value='/' />"><img
				src="${pageContext.request.contextPath}/images_index/logo-1.png"
			...
			<div class="collapse navbar-collapse" id="ftco-nav">
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item"><a
						href="<c:url value='/ACT/chooseSport' />" class="nav-link">賽事</a></li>
					<li class="nav-item"><a
						href="<c:url value='/product/index' />" class="nav-link">商城</a></li>
					<li class="nav-item"><a
						href="<c:url value='/DOS/DOS_Index' />" class="nav-link">場地</a></li>
					<c:if test="${!empty LoginOK}">
						<c:choose>
							<c:when test="${LoginOK.member_perm_id.member_perm_id==3 }">
								<li class="nav-item"><a
									href="<c:url value='/Back_index' />" class="nav-link">管理</a></li>
							</c:when>
							<c:when
								test="${LoginOK.member_perm_id.member_perm_id==1 || LoginOK.member_perm_id.member_perm_id==2}">
								<li class="nav-item"><a
									href="<c:url value='/member/memberInformation' />"
				...
				<div class="user">
					<c:if test="${empty LoginOK}">
						<a href="<c:url value='/member/loginForm' />" class="user-btn"><img
							src="${pageContext.request.contextPath}/images_index/user.png"
							alt="user" class="user-btn-img"> <span>登入</span> </a>
					</c:if>
					<c:if test="${! empty LoginOK}">
						<a href="<c:url value='/member/logout' />" class="user-btn"><img
							src="${pageContext.request.contextPath}/images_index/user.png"
							alt="user" class="user-btn-img"> <span>登出</span> </a>
					</c:if>
				</div>
			</div>
		</div>
