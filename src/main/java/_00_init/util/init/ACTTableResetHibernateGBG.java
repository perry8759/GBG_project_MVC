...
import com.web.GBG_project.ACT.dao.impl.ACTDaoImpl;
...
import com.web.GBG_project.member.dao.impl.MemberDaoImpl;
...
//		場地管理人|場地地址|容納人數|緯度|經度|場地名稱|場地聯絡人|場地費用|場地租借聯絡方式|場地備註|場地交通方式|場地運動種類編號
		try (InputStreamReader isr0 = new InputStreamReader(new FileInputStream(path + "dos.dat"), "UTF-8");
		...
		// -------------讀取DOS Pic資料，寫入資料庫----------------
		try (InputStreamReader isr0 = new InputStreamReader(new FileInputStream(path + "dos_pic.dat"), "UTF-8");
				BufferedReader br = new BufferedReader(isr0);) {
			tx = session.beginTransaction();
...
//		如果報錯: 開mysql的cmd，下指令: set global max_allowed_packet = 20*1024*1024;
		try (InputStreamReader isr0 = new InputStreamReader(new FileInputStream(path + "act.dat"), "UTF-8");
				BufferedReader br = new BufferedReader(isr0);) {
			tx = session.beginTransaction();
			while ((line = br.readLine()) != null) {
				String[] sa = line.split("\\|");
				...
				act.setACT_NEWS(commonUtils.StringToClob("暫無公告"));
...
//				act,MEMBER_ID,ACT_QES_COM
		try (InputStreamReader isr0 = new InputStreamReader(new FileInputStream(path + "qes.dat"), "UTF-8");
				BufferedReader br = new BufferedReader(isr0);) {
			tx = session.beginTransaction();
			while ((line = br.readLine()) != null) {
				String[] sa = line.split(",");
...
//		隊伍名稱|單位名稱|活動編號|報名狀態編號|會員id
	...
	public void follow() {
		...
		int[] id= {1,2,3,4,7,10,11};
		for(int actid:id) {
			ACT act = session.get(ACT.class, actid);
			Set<MemberBean> followers = act.getFollowers();
			int ra=(int)(Math.random()*85)+15;
			for(int memberid=15;memberid<ra;memberid++) {
