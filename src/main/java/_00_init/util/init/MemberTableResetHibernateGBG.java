...
public class MemberTableResetHibernateGBG {
	...
	public void loadFollower() {
		...
		try (
				InputStreamReader isr = new InputStreamReader(new FileInputStream(PATH + "memberFollower.dat"), "UTF-8");
				BufferedReader br = new BufferedReader(isr);
			) {
			tx = session.beginTransaction();
			int count = 0;
			System.out.println(">灌水單位帳=============================================================");
			//灌水單位帳
			line = br.readLine();
			...
			for(int i=9;i<16;i++) {
				...
				java.util.Date date = sdf2.parse(sa[10]);
				MemberBean memberBean = new MemberBean();
				memberBean.setMember_account("companyFollow"+count);
				memberBean.setMember_address(sa[1]);
				memberBean.setMember_birthday((sa[2].isEmpty() ? null : new Date(sdf1.parse(sa[2]).getTime())));
				memberBean.setMember_cp_name(sa[3]);
				memberBean.setMember_email(sa[4]);
				...
				memberBean.setMember_sex_id(session.get(MemberSexBean.class, Integer.valueOf(sa[16])));
				session.save(memberBean);
				System.out.println("單位member新增" + count + "筆記錄");
				count++;
			}
			System.out.println(">灌水一般帳=============================================================");
			//灌水一般帳
			line = br.readLine();
			System.out.println(line);
			count = 0;
...
//				java.util.Date date = sdf2.parse(sa[10]);
				MemberBean memberBean = new MemberBean();
//				memberBean.setMember_account("basicFollow"+count);
//				memberBean.setMember_address(sa[1]);
//				memberBean.setMember_birthday((sa[2].isEmpty() ? null : new Date(sdf1.parse(sa[2]).getTime())));
				memberBean.setMember_cp_name(sa[3]);
//				memberBean.setMember_email(sa[4]);
...
//				memberBean.setMember_tax_id_number(sa[11]);
				...
				memberBean.setMember_sex_id(session.get(MemberSexBean.class, Integer.valueOf(sa[16])));
				session.save(memberBean);
				System.out.println("一般member新增" + count + "筆記錄");
				count++;
			}
			tx.commit();
		} catch (Exception e) {
