...
public class ShoppingCartServiceImpl implements ShoppingCartService {
	...
	public void saveOrderDetail(MemberBean member, Map<String, String> orderMap, String orderId) {
		...
		for (ShoppingCartBean n : shoppingCart) {
			...
			ProductDetailBean productDetail = n.getProductDetailBean();
			...
			Integer purchase = productDetail.getProductBean().getProduct_purchases();
			if(purchase==null) {
				purchase=1;
			}else {
				purchase+=1;
			}
			productDao.updateProductPurchases(productDetail.getProductBean(), purchase);
		}
	}
	...
	public void saveOrder(MemberBean member, Map<String, String> orderMap, String orderId) {
		...
		Double totalPrice = Double.parseDouble(orderMap.get("totalPrice"));
		order.setAggregate_amount(totalPrice);
		dao.saveOrder(order);
	}
